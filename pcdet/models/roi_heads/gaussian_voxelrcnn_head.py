...
class GaussianVoxelRCNNHead(RoIHeadTemplate):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, num_class=1, **kwargs):
        ...
        # Shared Fully-Connected Layers before both classification and regression
        shared_fc_list = []
        ...
        # Regression-Log Variance Fully Connected Layers
        reg_std_fc_list = []
        for k in range(0, self.model_cfg.REG_STD_FC.__len__()):
            reg_std_fc_list.extend([
                nn.Linear(pre_channel, self.model_cfg.REG_STD_FC[k], bias=False),
                nn.BatchNorm1d(self.model_cfg.REG_STD_FC[k]),
                nn.ReLU()
            ])
            pre_channel = self.model_cfg.REG_STD_FC[k]

            if k != self.model_cfg.REG_STD_FC.__len__() - 1 and self.model_cfg.DP_RATIO > 0:
                reg_std_fc_list.append(nn.Dropout(self.model_cfg.DP_RATIO))
        self.reg_std_fc_layers = nn.Sequential(*reg_std_fc_list)
        # do not predict variance for theta
        self.reg_std_pred_layer = nn.Linear(pre_channel, (self.box_coder.code_size - 1) * self.num_class, bias=True)
        # self.reg_std_pred_act_layer = nn.Tanh()
        self.init_weights()

    def build_losses(self, losses_cfg):
        self.add_module(
            'reg_loss_func',
            loss_utils.KLDLoss(code_weights=losses_cfg.LOSS_WEIGHTS['code_weights'])
        )

    def init_weights(self):
        init_func = nn.init.xavier_normal_
        for module_list in [self.shared_fc_layer, self.cls_fc_layers, self.reg_fc_layers, self.reg_std_fc_layers]:
        ...
        nn.init.normal_(self.cls_pred_layer.weight, 0, 0.0001)
        nn.init.constant_(self.cls_pred_layer.bias, 0)
        nn.init.normal_(self.reg_pred_layer.weight, mean=0, std=0.0001)
        nn.init.constant_(self.reg_pred_layer.bias, 0)

        nn.init.xavier_normal_(self.reg_std_pred_layer.weight)
        nn.init.constant_(self.reg_std_pred_layer.bias, 0)

        # nn.init.kaiming_normal_(self.cls_pred_layer.weight, mode='fan_in')
        # nn.init.constant_(self.cls_pred_layer.bias, 0)
        # nn.init.kaiming_normal_(self.reg_pred_layer.weight, mode='fan_in')
        # nn.init.constant_(self.reg_pred_layer.bias, 0)
        # nn.init.kaiming_normal_(self.reg_std_pred_layer.weight, mode='fan_in')
        # nn.init.constant_(self.reg_std_pred_layer.bias, 0)
    ...
    def forward(self, batch_dict):
        ...
        rcnn_reg_std = self.reg_std_pred_layer(self.reg_std_fc_layers(shared_features))
        ...
        if not self.training:
            ...
            batch_dict['batch_box_std_preds'] = rcnn_reg_std
        else:
    ...
    def get_loss(self, tb_dict=None):
        ...
        # implement KL-Divergence and von-Mises loss for regression params
        rcnn_loss_reg, reg_tb_dict = self.get_box_reg_std_layer_loss(self.forward_ret_dict)
